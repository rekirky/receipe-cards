...
export default function CostingPdfDocument({ session }: Props) {
    ...
    <Document title={`Menu Costing — ${session.recipeName || 'Untitled'}`}>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.header}>
          ...
          {session.recipeName ? (
            <Text style={s.headerSub}>{session.recipeName}</Text>
          ) : null}
        </View>
        ...
        <View style={s.meta}>
          ...
          <View style={s.metaItem}>
            <Text style={s.metaLabel}>Portions:</Text>
            <Text style={s.metaValue}>{session.portionCount}</Text>
          </View>
          <View style={s.metaItem}>
            <Text style={s.metaLabel}>Ingredients:</Text>
            <Text style={s.metaValue}>{session.ingredients.length}</Text>
          </View>
        </View>
          ...
          {session.ingredients.map((ing) => {
            ...
            return (
              <View key={ing.id} style={s.row}>
                ...
                <Text style={s.cellPack}>
                  {ing.purchaseWeight} {formatUnit(ing.purchaseUnit)} / {formatCurrency(ing.purchaseCost)}
                </Text>
                <Text style={s.cellUsed}>
                  {ing.usedAmount} {formatUnit(ing.usedUnit)}
                </Text>
